...
Protein_ID=1;  % Insert the identity of the protein (Example: For 1BZ4, Protein_ID=1)


number_of_proteins=25;  % Number of protein in our data set
...
if helix_info==1   % If there is helices in protein
    
    %%  ***************   Pre-processing step *******************
    
    % ***********************************************************
    
    % ******************  Extract Alpha-Sticks  *****************
...
% **************************************************************
    ...
    number_all_helix=numel(pdb.Helix);  % number of helices in the pdb
    AtomNumber=numel(pdb.Model.Atom);  % number of atoms in the pdb
    start_residue_chain=[pdb.Helix.initSeqNum];  % array of all start residue
    ...
    for i=1:helix_number
        ...
        for j=1:AtomNumber
            ...
            if sw==1
                Matrix(cc,:)=s_e_coordinate; % matrix for all start-end coordinates
                cc=cc+1;
            end
        end
    ...
    % calculate all voxels for each helix and store in structure
        ...
        while (flag)
            for j=1:AtomNumber
                if ((pdb.Model.Atom(j).X==m) && (pdb.Model.Atom(j).Y==n) && (pdb.Model.Atom(j).Z==o))
                    ...
                    l=l+1;
                end  % only one iteration  (start voxel of each helix)
    ...
    tic;
    p=1;
    for i=1:helix_number  % number of helices
        helix_mat_pdb=[result_helix(i).Mat];
        s=size(helix_mat_pdb,1);
        start_coordinate_helix(p,:)=helix_mat_pdb(1,:); % a matrix for start voxels
        end_coordinate_helix(p,:)=helix_mat_pdb(s,:);  % a matrix for end voxels
        p=p+1;
    end
    
    % extract start and end voxel, then store in n*6 array
    ...
    % extract x(start,end) ,y(start,end), z(start,end)for all helices
    for i=1:helix_number
    ...
    % extract the start and end voxel of each stick
    
    p=1;
    for i=1:stick_number % number of indexes
        stick_mat=[result_stick(i).Mat];
        s=size(stick_mat,1);
        start_coordinate_stick(p,:)=stick_mat(1,:); %  matrix for start voxels
        end_coordinate_stick(p,:)=stick_mat(s,:);  %  matrix for end voxels
        p=p+1;
    end
    
    % extract start and end voxel and then store in n*6 array
    ...
    % extract x(start,end) ,y(start,end), z(start,end)for all sticks
    for i=1:stick_number
    ...
    % (Suppose each vector) |AB|
    
    for s=1:helix_number   % For all helices
    ...
    % length of each stick (Suppose each vector) |AB|
    for s=1:stick_number     % for all sticks
    ...
    for i=1:stick_number % number of sticks
        j=1;
    ...
    % compute the angle between each helix with other vectors 
    ...
    % compute the angle of each stick with other sticks
    ...
    %% Third Feature: Relative Length (RL)
    
    % **********************************************************************
    % *** compute the Relative length of each helix with other helices *****
    % **********************************************************************
    
    
    %  RL= abs(d1-d2)/d1+d2
    
    % phase1 : compute Relative length
    for i=1:helix_number
    ...
    % compute the Relative length of each stick with other sticks
    for i=1:stick_number
    ...
    % extend Relative Lengthh helix matrix
    for i=1:1
        for j=1:helix_number+1
    ...
    % extend Relative Length stick matrix
    for i=1:1
        for j=1:stick_number+1
    ...
    %  *****************   Evaluation Mac on three features (angle, RL, ED)   ***************
   ...
   %  *****************   Evaluation Bhattacharyya on three features (angle, RL, ED)   ***************
    ...
    paired_matrix=[];
    size_end=stick_number   %%%% if helix_numbers == stick_numbers 
    ...
    % *************  calculate result_mac_vote3_2_plc ********************
    ...
    if remove_helix_index~=0
        [final_correspondence_mac] = Adjust_index_SimVA(result_mac_vote3_2_plc,helix_number,remove_helix_index );
    end
    
    
    %  *****************   Evaluation SimVA-MAC    ***************************
    ...
    [ matrix_SimVA_mac,unassign_SimVA_mac,match_SimVA_mac,nmiscore_SimVA_mac,accurateScore_SimVA_mac,NetFmeasure_SimVA_mac,sensitivity_SimVA_mac,precision_SimVA_mac,Fmeasure_SimVA_mac,indF_SimVA_mac] = evaluate(topology_file_vector,final_correspondence_mac_vector)
    ...
    if remove_helix_index~=0
        [final_correspondence_bhattacharyya] = Adjust_index_SimVA( result_bhattacharyya_vote3_2_plc,helix_number,remove_helix_index );
    end
    
    
    
    %  *****************   Evaluation SimVA-bhattacharyya    ***************
    ...
    [ matrix_SimVA_bhat,unassign_SimVA_bhat,match_SimVA_bhat,nmiscore_SimVA_bhat,accurateScore_SimVA_bhat,NetFmeasure_SimVA_bhat,sensitivity_SimVA_bhat,precision_SimVA_bhat,Fmeasure_SimVA_bhat,indF_SimVA_bhat] = evaluate(topology_file_vector,final_correspondence_bhat_vector)
    ...
    accurateScore_SimVA_bhat_helix=accurateScore_SimVA_bhat;
    accurateScore_SimVA_mac_helix=accurateScore_SimVA_mac;
...
% ***********************************************************
if sheet_info==1   % If there is sheet in protein
    ...
    pdb=importdata(protein_data(Protein_ID).name);  % online: PDBStruct = getpdb(PDBid)
    number_all_strand=numel(pdb.Sheet);  % number of strands in the pdb
    AtomNumber=numel(pdb.Model.Atom);  % number of atoms in the pdb
    start_residue_chain=[pdb.Sheet.initSeqNum];  % array of all start residues
    end_residue_chain=[pdb.Sheet.endSeqNum];    % array of all end residues
    resSeq=[pdb.Model.Atom.resSeq]; % Residue in Atom number
    ...
    % calculate all voxel for each strand and store in structure
        ...
        while (flag)
            for j=1:AtomNumber
                if ((pdb.Model.Atom(j).X==m) && (pdb.Model.Atom(j).Y==n) && (pdb.Model.Atom(j).Z==o))
                    ...
                    next=new_start_residue(i)+1;
                    l=l+1;
                end  % only one iteration  (start voxel of each strand)
    ...
    tic;
    p=1;
    for i=1:strand_number  % number of strands
        strand_mat_pdb=[result_strand(i).Mat];
        s=size(strand_mat_pdb,1);
        start_coordinate_strand(p,:)=strand_mat_pdb(1,:); % a matrix for start voxels
        end_coordinate_strand(p,:)=strand_mat_pdb(s,:);  % a matrix for end voxels
        p=p+1;
    end
    
    % extract start and end voxels, then store in n*6 array
    ...
    % extract x(start,end) ,y(start,end), z(start,end)for all strands
    for i=1:strand_number
    ...
    % calculate the start and end voxel of each Beta stick
    p=1;
    for i=1:stick_number % number of indexes
        stick_mat=[result_stick(i).Mat];
        s=size(stick_mat,1);
    ...
    for i=1:strand_number % number of beta strands
        j=1;
    ...
    for i=1:stick_number % number of beta sticks
        j=1;
        x1=start_coordinate_stick(i,j);
        y1=start_coordinate_stick(i,j+1);
        z1=start_coordinate_stick(i,j+2);
    ...
    % compute the angle of each strand with other strands
    for i=1:strand_number
    ...
    % **********************************************************************
    % *** compute the Relative length of each strand with other strands ****
    % **********************************************************************
    ...
    %%  Compute MAC and Bhattacharya Score For Strands
    %******************************************************
    ...
    paired_matrix=[];
    size_end=stick_number   %%%% if strand_number == stick_number
    ...
    if remove_strand_index~=0
        [final_correspondence_mac_strand] = Adjust_index_SimVA(result_mac_vote3_2_plc_strand,strand_number,remove_strand_index );
    end
    ...
    [ matrix_SimVA_mac_strand,unassign_SimVA_mac_strand,match_SimVA_mac_strand,nmiscore_SimVA_mac_strand,accurateScore_SimVA_mac_strand,NetFmeasure_SimVA_mac_strand,sensitivity_SimVA_mac_strand,precision_SimVA_mac_strand,Fmeasure_SimVA_mac_strand,indF_SimVA_mac_strand] = evaluate(topology_file_vector_strand,final_correspondence_mac_vector_strand)
    ...
    if remove_strand_index~=0
        [final_correspondence_bhattacharyya_strand] = Adjust_index_SimVA( result_bhattacharyya_vote3_2_plc_strand,strand_number,remove_strand_index );
    end
    
    
    
    %  *****************   Evaluation SimVA-bhattacharyya (BD)    ***************
    ...
    [ matrix_SimVA_bhat_strand,unassign_SimVA_bhat_strand,match_SimVA_bhat_strand,nmiscore_SimVA_bhat_strand,accurateScore_SimVA_bhat_strand,NetFmeasure_SimVA_bhat_strand,sensitivity_SimVA_bhat_strand,precision_SimVA_bhat_strand,Fmeasure_SimVA_bhat_strand,indF_SimVA_bhat_strand] = evaluate(topology_file_vector_strand,final_correspondence_bhat_vector_strand)
    ...
    accurateScore_SimVA_bhat_strand=accurateScore_SimVA_bhat_strand;
    accurateScore_SimVA_mac_strand=accurateScore_SimVA_mac_strand;
...
if helix_info==1
    ...
    clc;
    disp ('**************************************************************');
    disp ('************** Results for Alpha-Helices *********************');
    disp ('**************************************************************');
    disp (' ');
    disp ('Accuracy Angle using BD for helices:');
    disp (100*accurateScore_angle_bhat_helix)
    
    disp ('Accuracy Euclidean Distance(ED)using BD for helices:');
    disp (100*accurateScore_Euclidean_bhat_helix)
    
    disp ('Accuracy Relative Length(RL) using BD for helices:');
    disp (100*accurateScore_deltaLength_bhat_helix)
     
    disp ('Accuracy Total Features using BD for helices:');
    disp (100*(accurateScore_angle_bhat_helix+accurateScore_deltaLength_bhat_helix+accurateScore_Euclidean_bhat_helix)/3)
    
    
    disp('***************************************************************')
    
    disp ('Accuracy Angle using MAC for helices:');
    disp (100*accurateScore_angle_mac_helix);
    
    disp ('Accuracy Euclidean Distance (ED)using MAC for helices:');
    disp (100*accurateScore_Euclidean_mac_helix)
    
    disp ('Accuracy Relative Length(RL)using MAC for helices:');
    disp (100*accurateScore_deltaLength_mac_helix)
    
    
    disp ('Accuracy Total Features using MAC for helices:');
    disp (100*(accurateScore_angle_mac_helix+accurateScore_deltaLength_mac_helix+accurateScore_Euclidean_mac_helix)/3)
    
    
    
    disp('***************************************************************')
    disp ('Accuracy SimVA using MAC for helices:');
    disp (100*accurateScore_SimVA_bhat_helix)
    
    disp ('Accuracy SimVA using MAC for helices');
    disp (100*accurateScore_SimVA_mac_helix)
    ...
    disp ('**************************************************************');
    disp ('********  Results of SimVA Algorithm for Helices *************');
    disp ('**************************************************************');
    
    disp ('Precision SimVA using BD for helices:');
    disp (100*precision_SimVA_bhat);
    disp ('Sensitivity SimVA using BD for helices:');
    disp (100*sensitivity_SimVA_bhat);
    disp ('Fmeasure SimVA using BD for helices:');
    disp (100*Fmeasure_SimVA_bhat);
    disp ('***************************************************************');
    
    disp ('Precision SimVA using MAC for helices:');
    disp (100*precision_SimVA_mac);
    disp ('Sensitivity SimVA using MAC for helices:');
    disp (100*sensitivity_SimVA_mac);
    disp ('Fmeasure SimVA using MAC for helices:');
    disp (100*Fmeasure_SimVA_mac);
...
if sheet_info==1
    ...
    disp ('**************************************************************');
    disp ('************** Results for Beta-Sheets ***********************');
    disp ('**************************************************************');
    disp (' ');
    
    disp ('Accuracy Angle using BD for Strands:');
    disp (100*accurateScore_angle_bhat_strand)
    
    disp ('Accuracy Euclidean Distance(ED) using BD for Strands:');
    disp (100*accurateScore_Euclidean_bhat_strand)
    
    disp ('Accuracy Relative Length(RL) using BD for Strands:');
    disp (100*accurateScore_deltaLength_bhat_strand)
    
    
    disp ('Accuracy Total Features using BD for Strands:');
    ...
    disp ('Accuracy Angle using MAC for Strands:');
    disp (100*accurateScore_angle_mac_strand);
    
    disp ('Accuracy Euclidean Distance(ED)using MAC for Strands:');
    disp (100*accurateScore_Euclidean_mac_strand)
    
    
    disp ('Accuracy Relative Length(RL)using MAC for Strands:');
    disp (100*accurateScore_deltaLength_mac_strand)
    
    
    disp ('Accuracy Total Features using MAC for Strands:');
    disp (100*(accurateScore_angle_mac_strand+accurateScore_deltaLength_mac_strand+accurateScore_Euclidean_mac_strand)/3)
    
    
    disp('****************************************************************')
    disp ('Accuracy SimVA algorithm using BD for Strands');
    disp (100*accurateScore_SimVA_bhat_strand)
    
    disp ('Accuracy SimVA algorithm using MAC for Strands');
    disp (100*accurateScore_SimVA_mac_strand)
    ...
    disp ('***************************************************************');
    disp ('*******  Results of SimVA algorithm for strands **************');
    disp ('***************************************************************');
    
    disp ('Precision SimVA using BD for Strands:');
    disp (100*precision_SimVA_bhat_strand);
    disp ('Sensitivity SimVA using BD for Strands:');
    disp (100*sensitivity_SimVA_bhat_strand);
    disp ('Fmeasure SimVA using BD for Strands:');
    disp (100*Fmeasure_SimVA_bhat_strand);
    disp ('****************************************************************');
    
    disp ('Precision SimVA using MAC for Strands:');
    disp (100*precision_SimVA_mac_strand);
    disp ('Sensitivity SimVA using MAC for Strands:');
    disp (100*sensitivity_SimVA_mac_strand);
    disp ('Fmeasure SimVA using MAC for Strands:');
    disp (100*Fmeasure_SimVA_mac_strand);
...
if helix_info==1 && sheet_info==1
    ...
    accurateScore_SimVA_bhat_SSE=(accurateScore_SimVA_bhat_helix+accurateScore_SimVA_bhat_strand)/2;
    accurateScore_SimVA_mac_SSE=(accurateScore_SimVA_mac_helix+accurateScore_SimVA_mac_strand)/2;
    
    
    precision_SimVA_bhat_SSE=(precision_SimVA_bhat+precision_SimVA_bhat_strand)/2;
    sensitivity_SimVA_bhat_SSE=(sensitivity_SimVA_bhat+sensitivity_SimVA_bhat_strand)/2;
    Fmeasure_SimVA_bhat_SSE=(Fmeasure_SimVA_bhat+Fmeasure_SimVA_bhat_strand)/2;
    
    precision_SimVA_mac_SSE=(precision_SimVA_mac+precision_SimVA_mac_strand)/2;
    sensitivity_SimVA_mac_SSE=(sensitivity_SimVA_mac+sensitivity_SimVA_mac_strand)/2;
    Fmeasure_SimVA_mac_SSE=(Fmeasure_SimVA_mac+Fmeasure_SimVA_mac_strand)/2;
  
    clc;

    
    disp ('**************************************************************');
    disp ('********************* Results for  SSEs **********************');
    disp ('***************************************************************');
    disp ('Total Time for Matching SSEs:');
    disp (total_time);
    
    disp ('Accuracy Angle using BD for SSEs:');
    disp (100*accurateScore_angle_bhat_SSE);
    disp ('Accuracy Euclidean Distance(ED) using BD for SSEs:');
    disp (100*accurateScore_Euclidean_bhat_SSE);
    disp ('Accuracy Relative Length(RL) using BD for SSEs:');
    disp (100*accurateScore_deltaLength_bhat_SSE);
    
    
    disp ('Accuracy Total Features using BD for SSEs:');
    disp (100*(accurateScore_angle_bhat_SSE+accurateScore_deltaLength_bhat_SSE+accurateScore_Euclidean_bhat_SSE)/3)
    
    disp ('****************************************************************');

    
    disp ('Accuracy Angle using MAC for SSEs:');
    disp (100*accurateScore_angle_mac_SSE);
    disp ('Accuracy Euclidean Distance(ED) using MAC for SSEs:');
    disp (100*accurateScore_Euclidean_mac_SSE);
    disp ('Accuracy Relative Length(RL) using MAC for SSEs:');
    disp (100*accurateScore_deltaLength_mac_SSE);
    
    
    disp ('Accuracy Total Features using MAC for SSEs:');
    disp (100*(accurateScore_angle_mac_SSE+accurateScore_deltaLength_mac_SSE+accurateScore_Euclidean_mac_SSE)/3)
    
    disp ('****************************************************************');
  
    
    disp ('Accuracy SimVA using BD for SSEs:');
    disp (100*accurateScore_SimVA_bhat_SSE);
    disp ('Accuracy SimVA using MAC for SSEs:');
    disp (100*accurateScore_SimVA_mac_SSE);
    
    
    disp ('**************************************************************');
    disp ('******  Results of SimVA algorithm for SSES ******************');
    disp ('**************************************************************');
    
    disp ('Precision SimVA using BD for SSEs:');
    disp (100*precision_SimVA_bhat_SSE);
    disp ('Sensitivity SimVA using BD for SSEs:');
    disp (100*sensitivity_SimVA_bhat_SSE);
    disp ('Fmeasure SimVA using BD for SSEs:');
    disp (100*Fmeasure_SimVA_bhat_SSE);
    disp ('**************************************************************');
    
    
    disp ('Precision SimVA using MAC for SSEs:');
    disp (100*precision_SimVA_mac_SSE);
    disp ('Sensitivity SimVA using MAC for SSEs:');
    disp (100*sensitivity_SimVA_mac_SSE);
    disp ('Fmeasure SimVA using MAC for SSEs:');
    disp (100*Fmeasure_SimVA_mac_SSE); 
